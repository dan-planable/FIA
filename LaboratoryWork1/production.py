...
def forward_chain(rules, data, apply_only_one=False, verbose=False):
    ...
    """
    old_data = ()  # Initialize a variable to store the previous state of data

    while set(old_data) != set(data):
        old_data = list(data)  # Copy the current state of data
...
def backward_chain(rules, hypothesis, verbose=False):
    ...
    """
    # Initialize a stack with the initial hypothesis
    goal_stack = [hypothesis]  
    # Initialize an empty goal tree to store the reasoning steps
    goal_tree = []  

    while goal_stack:
        current_goal = goal_stack.pop()  # Get the current goal from the stack

    # Initialize a variable to store the rule that matches the goal
        found_rule = None  
        for rule in rules:
            # Attempt to match the current goal with the consequent of each rule
            match_res = match(rule.consequent()[0], current_goal)
            if match_res:
                found_rule = rule  # If a matching rule is found, store it
                break

        if found_rule:
            goal_tree.append(found_rule.antecedent())  # Add the antecedent of the found rule to the goal tree
            for antecedent in found_rule.antecedent():
                goal_stack.append(populate(antecedent, match_res))  # Populate and add antecedents to the stack
